...
## 2026-06-24 — M47: 컨트롤 스틱을 HOTAS 형상으로 재모델링

**Status**: GREEN (HOTAS 형상 + 전방 개방 + 디플렉션 확인, 194 통과, 콘솔 0, 착륙 불변)
**Files changed**: src/cockpit.js(스틱 재모델링), src/main.js(__ckStick→assembly)
**Tests**: 194 통과, 콘솔 0, 6 m/s+2.5 거스트 착륙 PASS(110m, 불변)
**Decisions**:
- 피드백: 기존 스틱(가는 샤프트+둥근 캡) 실루엣이 민망함 → 레퍼런스(Thrustmaster HOTAS)풍으로 재모델링.
- **고정 베이스 콘솔**(8각 플레이트+실버 림+오렌지 짐벌 링+오렌지/시안 콘솔 버튼) + **틸팅 스틱**(고무 부트 → 짧은 메탈 넥 → **납작한 에르고 그립**(BoxGeometry, 폭/깊이가 커서 기둥형 아님)+실버 프론트+트리거 → **샤프트보다 넓은 둥근 버튼 헤드**(햇 스위치, 오렌지 버튼 2, 썸 스틱)). 넓은 헤드가 조이스틱/운전대 실루엣을 만들어 민망함 제거.
- 구조 분리: `assembly`(위치/스케일, `__ckStick`이 제어) ⊃ 고정 base + `stick`(틸팅, updateCockpit이 controls.pitch/roll로 회전). 스윕으로 scale 0.95 / base(0,-0.82,-0.55) — 하단에 또렷하되 수평선 개방.
**Next**:
