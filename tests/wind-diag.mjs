...
const expr = `(function(){ const r=window.__advance(1); const h=window.__hils; const d=h.diag;
  return {st:r.status, ph:h.auto.phase, alt:+(h.pos.y-0.8).toFixed(1), x:+h.pos.x.toFixed(0), gs:+h.vel.spd.toFixed(1),
    as:d.airspd, aoa:d.aoa, pit:d.pitch, bnk:d.bank, thr:d.thr, fl:d.flaps, wx:+h.wind.x.toFixed(1), wz:+h.wind.z.toFixed(1)};})()`;
let printing = false;
for (let t = 0; t < 200; t++) {
  const s = await ev(expr);
  if (s.ph === 'APPROACH') printing = true;
  if (printing) console.log(`t${t} ${s.ph} alt=${s.alt} x=${s.x} gs=${s.gs} AS=${s.as} aoa=${s.aoa} pit=${s.pit} bnk=${s.bnk} thr=${s.thr} fl=${s.fl} w=(${s.wx},${s.wz}) ${s.st}`);
